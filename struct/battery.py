class Battery:
  ...
  def checkBatteryState(self):
      print(f"Battery state: {self.battery_state}/{self.lower_limit} kWh power ready: {(self.battery_state-self.lower_limit)*self.multiplier} kW")
      self.insertDataToTable()
      return self.battery_state
  
  def chargeBattery(self, available, power_to_return):
    if available < self.upper_limit:
        self.updateBattery(available - power_to_return, True)
