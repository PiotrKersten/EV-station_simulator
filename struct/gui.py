...
class Gui:
    ...
    def insertSimulationParameter(self):
        ...
        tk.Label(frame1, text="Single charger power [kW]:", font=font).pack()
        entry14 = tk.Entry(frame1)
        entry14.insert(0, default_chargers_power) 
        entry14.pack()

        tk.Label(frame2, text="Desired charging time [t.u]:", font=font).pack()
        entry2 = tk.Entry(frame2)
        entry2.insert(0, default_charging_time)  
        entry2.pack()

        tk.Label(frame3, text="Generate EV arriving time [t.u]:", font=font).pack()
        ...
        tk.Label(frame4, text="Or give average time [t.u]:", font=font).pack()
        entry41 = tk.Entry(frame4)
        entry41.insert(0, default_average_time) 
        entry41.pack()

        tk.Label(frame4, text="Battery capacity [kWh]:", font=font).pack()
        entry5 = tk.Entry(frame4)
        entry5.insert(0, default_battery_capacity)
        entry5.pack()
        
        tk.Label(frame4, text="Grid power [kW]:", font=font).pack()
        entry51 = tk.Entry(frame4)
